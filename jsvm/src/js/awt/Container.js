...
js.awt.Container = function (def, Runtime, view){
	 ...
	 * visible components and the container itself.
	 * 
	 * @return {Object}
	 *
	 * @link js.swt.SComponent#getScrollSize
	 */
	thi$.getScrollSize = function(){
		var D = this.getBounds(), MBP = D.MBP, items = this.items0(),
			w = 0, h = 0, i, len, comp, box;
		...
		for(i = 0, len = items.length; i < len; i++){
			...
			// About the scroll region:
			// in chrome, exclude the margin-rigth
			// in firefox and ie 9+, exclude the margin-bottom and margin-right
			// Ref: webos/app/demo/margin.html
			//
			// So, don't sum any margin-right and margin-bottom of content region
			// as current container's scrollable size.
			box = comp.getBounds();
