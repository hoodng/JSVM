...
js.awt.Item = function(def, Runtime, view){
    ...
    thi$.setIconImage = function(state){
        ...
        if(M.useBgImage !== true){
            ...
            if(M.iconImage){
                styles = {
                    backgroundImage: "url(" + buf.toString() + ")",
    ...
    thi$.paintShapeSign = function(signObj){
    ...
    thi$.isMoverSpot = function(el, x, y){
        return $super(this) && el != this.branch
            && el != this.marker && el !== this.ctrl;
    ...
    thi$.onStateChanged = function(){
        ...
        if(this.isStrict() && icon){
            ...
            if(!this.isEnabled()){
                styleClass += " "
                    + DOM.combineClassName(icon.clazz, "--disabled", "");
            }
