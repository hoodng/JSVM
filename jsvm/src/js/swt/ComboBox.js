...
js.swt.ComboBox = function(def, Runtime){
    ...
    thi$.setSearchCritical = function(num){
        if(Class.isNumber(num)){
            ...
            if(this.subview){
                this.subview.setSearchCritical(num);
            }
        }
    };
    ...
    thi$.onMoved = function(){
        ...
        if(this.subview){
            this.subview.invalidateLayout(false);
        }
    ...
    thi$.destroy = function(){
        ...
        if(this.subview){
            _showSubview.call(this, false);
            this.subview.setPMFlag(0);
            
            this.subview.detachEvent(List.EVT_READY, 4, this, _onListEvent);
            this.subview.detachEvent(List.EVT_ACK_ITEMSADDED, 4, this, _onListEvent);
            this.subview.detachEvent(List.EVT_ACK_ITEMSREMOVED, 4, this, _onListEvent);
            this.subview.detachEvent(DDList.EVT_SUBMITVALUES, 4, this, _onListSubmit);
    ...
    var _doEdit = function(){
        var curV = this._inputView.getValue();
        
        this._inputView.detachEvent(js.swt.TextField.EVT_SUBMIT, 4, this, _doEdit);
        this.dItemContainer.removeComponent(this._inputView);
        this._inputView.destroy();
         ...
         */
        if(typeof this.rectifyInput == "function"){
        ...
        if(this.subview){
            this.subview.setSelectedValues(values, false);
        }
    ...
    var _preIptStyles = function(){
        var latestStyles = this._latestStyles, styles = {}, sp, v;
        if(!latestStyles){
            return styles;
        }
        
        for(var i = 0, len = iptSps.length; i < len; i++){
            sp = iptSps[i];
            v = latestStyles[sp];
    ...
    var _onEdit = function(e){
        ...
        this._local.latestModel = this.displayItem.model;

        // Record the styles and remove the displayItem
        _removeDisplayItem.call(this);
        
        // Create input box
        var Clz = Class.forName("js.swt.TextField"),
        ...
        iptDef.css = (iptDef.css || "") + DOM.toCssText(iptStyles);
        
        var input = this._inputView = 
            new (Clz)(iptDef, this.Runtime());

        var m = this._local.latestModel,
        v = this._latestValue 
            = (Class.isObject(m) ? m.dname || "" : "");
        input.setValue(v);
        this.dItemContainer.addComponent(input);
        this.dItemContainer.doLayout(true);
        
        input.attachEvent(Clz.EVT_SUBMIT, 4, this, _doEdit);
        input.focus(true);
    };
    ...
    var _removeDisplayItem = function(){
        if(this.displayItem){
            if(!this._latestStyles){
                this._latestStyles = this.displayItem.getStyles(sps);
            }
            
            this._latestWidth = this.displayItem.getWidth();
            this._latestHeight = this.displayItem.getHeight();
            
            if(this.editable) {
                this.displayItem.detachEvent("click", 0, this, _onEdit);
            }
            
            this.dItemContainer.removeComponent(this.displayItem);
            this.displayItem.destroy();
        }
    ...
    var _onLayerRemoved = function(e){
        ...
        eType = evt ? evt.getType() : undefined,
        ...
        if(type == "afterRemoveLayer"){
            ...
            if(eType == "hide" || (el && !trigger.contains(el, true))){
                this._local.expectedOp = 1; 
                this.setHover(false);
            }
        }
    ...
    var _quitEdit = function(){
        if(!this.editable || !this._inputView) return;
